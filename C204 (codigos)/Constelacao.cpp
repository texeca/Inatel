...
int main()
{
	int n;
	int soma = 0;
	list<ponto> ::iterator j;//uma lista que funciona basicamente como um ponteiro
	ponto v[100];//chamar o struct do início da função para declara o vetor
	cin >> n;
	for(int i = 0; i < n; i++)
	{
		cin >> v[i].x;
	...
	list<ponto> resp;//declaração de lista
	resp = Graham_scan(v, n);//recebr a lista da função graham
	for(j = resp.begin(); j != resp.end(); j++) //vendo o início da lista com o ponteiro J, e se o J for diferente do final de resp, o for para
	{
		soma++;
	}
	cout << soma << " lados" << endl;
	return 0;
}
