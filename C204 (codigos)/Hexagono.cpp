...
{
	float resp;
	ponto v[100];//vetor para armazenar os valores do polígono
	for(int i = 0; i < 6; i++)
	{
		cin >> v[i].x;//entrar com a varável, um ponto , e a variável para preencher
		cin >> v[i].y;
	}
	resp = area_poligono(v, 6);
	cout << fixed << setprecision(2) << "Area do escudo: " << resp << endl;
	return 0;
}
